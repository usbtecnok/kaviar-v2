  ...
  if (loading) {
    return (
      <Container maxWidth="lg" sx={{ py: 4, textAlign: 'center' }}>
        ...
        <Typography sx={{ mt: 2, color: '#6B7280' }}>Carregando tabela de preços...</Typography>
      </Container>
    );
  }
  ...
  return (
    <Container maxWidth="lg" sx={{ py: 2 }}>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
        <Tabs value={tab} onChange={(_, v) => setTab(v)} sx={{ '& .MuiTab-root': { fontWeight: 600, fontSize: 13, textTransform: 'none' }, '& .Mui-selected': { color: `${GOLD} !important` }, '& .MuiTabs-indicator': { bgcolor: GOLD } }}>
          <Tab label={`Preços Ativos (${floors.length})`} />
          <Tab label={`Minhas Propostas (${proposals.length})`} />
        </Tabs>
        <Button startIcon={<Add />} variant="contained" size="small" onClick={() => setProposeDialog(true)} sx={{ bgcolor: GOLD, '&:hover': { bgcolor: '#9A7B24' }, fontSize: 12 }}>
      ...
      {tab === 0 && (
        floors.length === 0 ? (
          <Card sx={{ border: '1px solid #E8E5DE', borderRadius: 2 }}>
            ...
            <Table size="small">
              <TableHead sx={{ bgcolor: '#FAFAF8' }}>
                <TableRow>
                  ...
                  <TableCell sx={{ fontWeight: 700, fontSize: 11 }} align="right">Piso Mínimo</TableCell>
                  ...
                  <TableCell sx={{ fontWeight: 700, fontSize: 11 }}>Observação</TableCell>
                </TableRow>
              </TableHead>
              <TableBody>
                {floors.map(f => (
            ...
            {proposals.map(p => {
              ...
              return (
                <Grid item xs={12} sm={6} md={4} key={p.id}>
                  <Card sx={{ border: `1px solid ${STATUS_COLORS[p.status] || '#E8E5DE'}30`, borderRadius: 2, borderTop: `3px solid ${STATUS_COLORS[p.status] || '#E8E5DE'}` }}>
                    <CardContent sx={{ py: 2, px: 2 }}>
                      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 1 }}>
                        <Typography sx={{ fontWeight: 700, fontSize: 12 }}>{p.origin_label} → {p.dest_label}</Typography>
                        <Chip
                      ...
                      <Typography sx={{ fontSize: 13, color: GOLD, fontWeight: 700, mb: 0.5 }}>
                        {fmt(p.total_floor)}
                        {p.surcharge > 0 && <span style={{ fontSize: 11, color: '#6B7280', fontWeight: 400 }}> (piso {fmt(p.floor_price)} + acréscimo {fmt(p.surcharge)})</span>}
                      </Typography>
                      <Typography sx={{ fontSize: 11, color: '#6B7280', mb: 0.5 }}>Justificativa: {p.notes}</Typography>
                      {p.review_reason && (
                        <Alert severity={p.status === 'rejected' ? 'error' : 'info'} sx={{ mt: 1, py: 0, fontSize: 11 }}>
      ...
      <Dialog open={proposeDialog} onClose={() => setProposeDialog(false)} maxWidth="sm" fullWidth>
        <DialogTitle sx={{ fontWeight: 700 }}>Solicitar Novo Preço</DialogTitle>
        <DialogContent>
        ...
        <DialogActions>
          ...
          <Button onClick={handlePropose} variant="contained" sx={{ bgcolor: GOLD, '&:hover': { bgcolor: '#9A7B24' } }}>Enviar Proposta</Button>
        </DialogActions>
      </Dialog>
    </Container>
  );
