                ...
                <CardContent sx={{ py: 2 }}>
                  ...
                  <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <Box sx={{ display: 'flex', gap: 0.5 }}>
                      ...
                      {o.status === 'READY' && o.delivery_type === 'delivery' && o.delivery_status === 'none' && <Button size="small" variant="contained" onClick={async () => { const res = await fetch(`${API_BASE_URL}/api/commerce/orders/${o.id}/request-delivery`, { method: 'POST', headers }); const d = await res.json(); if (d.success) { fetchOrders(); setSnack('Entrega solicitada!'); } else setSnack(d.error || 'Erro'); }} sx={{ bgcolor: '#059669', textTransform: 'none', fontSize: 11 }}>🚗 Solicitar Entrega</Button>}
                      {o.delivery_status === 'requested' && <Chip label="Aguardando motorista" size="small" color="warning" sx={{ fontSize: 10 }} />}
