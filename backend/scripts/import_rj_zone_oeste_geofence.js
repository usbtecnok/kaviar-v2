...
async function importZoneOesteGeofenceData() {
  const isDryRun = process.env.DRY_RUN === 'true';
  
  console.log(`🗺️ ${isDryRun ? 'SIMULANDO' : 'Importando'} dados de geofence da Zona Oeste...`);
  
  // Ler dados da pesquisa Zona Oeste
  const auditFile = path.join(__dirname, '..', '..', 'audit', 'rj_zone_oeste_geofences_raw.json');
  
  if (!fs.existsSync(auditFile)) {
    throw new Error(`Arquivo de pesquisa Zona Oeste não encontrado: ${auditFile}`);
  }
  ...
  for (const item of geofenceData) {
    try {
      ...
      // REGRA: Fuzzy match ou LOW confidence não importa automaticamente
      if (item.confidence === 'LOW') {
        console.log(`⚠️ Confidence LOW bloqueado: ${item.name}`);
        importReport.push({
          name: item.name,
          status: 'low_confidence_blocked',
          confidence: item.confidence,
          reason: 'Confidence LOW - requer revisão manual'
        });
        skipped++;
        continue;
      }
  ...
  const reportPath = path.join(__dirname, '..', '..', 'audit', 'rj_zone_oeste_geofence_import_report.md');
  
  let report = `# Relatório de Importação Geofence - Zona Oeste/Barra + Alto da Boa Vista\n\n`;
  report += `**Data:** ${new Date().toISOString()}\n`;
  report += `**Modo:** ${isDryRun ? 'DRY-RUN (Simulação)' : 'IMPORT REAL'}\n`;
  report += `**Total processado:** ${geofenceData.length}\n\n`;
  
  report += '## Resumo\n\n';
  report += `- **${isDryRun ? 'Seriam importados' : 'Importados'}:** ${imported}\n`;
  ...
  const byStatus = {
    ...
    low_confidence_blocked: importReport.filter(r => r.status === 'low_confidence_blocked').length,
  ...
  report += `- **LOW confidence bloqueados:** ${byStatus.low_confidence_blocked}\n`;
  ...
  report += `- **LOW:** ${byConfidence.LOW} (bloqueados para revisão manual)\n\n`;
  ...
  importReport.forEach(item => {
    const status = item.status === 'would_import' ? 'Importaria' : 
                   ...
                   item.status === 'low_confidence_blocked' ? 'LOW bloqueado' :
...
async function main() {
  try {
    await importZoneOesteGeofenceData();
  } catch (error) {
...
module.exports = { importZoneOesteGeofenceData };
