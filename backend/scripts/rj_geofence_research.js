...
async function searchGeofence(community) {
  ...
  try {
    ...
    const response = await fetch(url, {
      headers: {
        'User-Agent': 'KAVIAR-RJ-Geofence-Research/1.0'
      }
    });
    ...
    // Determinar confiança baseada na qualidade dos dados
    let confidence = 'MED';
...
async function main() {
  console.log('🌍 Iniciando pesquisa de geofence para RJ...');
  
  // Buscar todas as comunidades do RJ
  ...
  console.log(`📊 ${communities.length} comunidades encontradas no RJ`);
  ...
  // Salvar JSON bruto
  fs.writeFileSync(
    path.join(auditDir, 'rj_geofences_raw.json'),
  ...
  fs.writeFileSync(
    path.join(auditDir, 'rj_geofences.geojson'),
    JSON.stringify(geojson, null, 2)
  );
  
  // Gerar relatório markdown
  let report = '# Relatório de Pesquisa Geofence - Rio de Janeiro\n\n';
  ...
  const stats = {
    ...
    neighborhoods: results.filter(r => r.description && !r.description.includes('Morro')).length,
    communities: results.filter(r => r.description && r.description.includes('Morro')).length
  };
  ...
  fs.writeFileSync(
    path.join(auditDir, 'rj_geofence_research_report.md'),
    report
  );
