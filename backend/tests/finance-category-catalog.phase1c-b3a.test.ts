...
describe('finance category catalog phase 1C-B 3A', () => {
  it('matches the required structure and catalog invariants', () => {
    ...
    expect(categories.filter((category) => category.is_active !== false)).toHaveLength(41);
    expect(categories.filter((category) => category.is_active === false)).toHaveLength(10);
    expect(categories.filter((category) => !category.parent_code)).toHaveLength(13);
    expect(categories.filter((category) => category.parent_code).length).toBe(38);
