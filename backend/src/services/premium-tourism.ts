...
export class PremiumTourismService {
   ...
   */
  async createTourBooking(data: TourBookingData): Promise<{ booking: TourBookingResponse; premiumDriversAvailable: number }> {
    if (!this.isPremiumTourismEnabled()) {
      throw new FeatureDisabledError();
    }
    ...
    // Audit log
    console.log(`[TOUR_BOOKING_CREATED] BookingId: ${booking.id}, PackageId: ${data.packageId}, PassengerId: ${data.passengerId}`);

    return {
      booking: this.mapTourBookingResponse(booking),
   ...
   */
  async confirmTourBooking(bookingId: string, adminId: string): Promise<{ booking: TourBookingResponse; rideId: string }> {
    if (!this.isPremiumTourismEnabled()) {
      throw new FeatureDisabledError();
    }
    ...
    return await prisma.$transaction(async (tx) => {
      ...
      if (booking.status !== TourBookingStatus.REQUESTED) {
      ...
      if (!availability.hasAvailable) {
        throw new Error('No premium drivers available');
      }

      // Create ride with TOURISM type
      const ride = await tx.ride.create({
        data: {
          passengerId: booking.passengerId,
          origin: booking.pickupLocation,
          destination: booking.dropoffLocation || 'Tour destination',
          type: 'TOURISM', // New ride type for premium tourism
          price: booking.package.basePrice,
          status: 'requested'
        }
      });

      // Update booking to confirmed with ride link
      const updatedBooking = await tx.tourBooking.update({
        where: { id: bookingId },
        data: {
      ...
      // Audit log
      console.log(`[TOUR_BOOKING_CONFIRMED] AdminId: ${adminId}, BookingId: ${bookingId}, RideId: ${ride.id}`);

      return {
        booking: this.mapTourBookingResponse(updatedBooking),
        rideId: ride.id
      };
    });
  }
