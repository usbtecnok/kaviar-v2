...
router.get('/me/credits/packages', authenticateDriver, async (_req: Request, res: Response) => {
  try {
    const { rows } = await pool.query(
      'SELECT id, name, credits_amount, price_cents FROM credit_packages WHERE active = true ORDER BY credits_amount'
    );
    res.json({ success: true, data: rows });
  } catch (err: any) {
    console.error('[CREDIT_PACKAGES]', err);
...
router.post('/me/credits/purchase', authenticateDriver, async (req: Request, res: Response) => {
  try {
    ...
    // Cria cobrança Pix
    const pix = await createPixPayment(customerId, p.price_cents, extRef, `KAVIAR: ${p.name}`);
    ...
    await pool.query(
      ...
      [driverId, p.id, customerId, pix.paymentId, p.price_cents, p.credits_amount, extRef, pix.qrCode, pix.copyPaste, pix.expirationDate]
    );

    console.log(`[CREDIT_PURCHASE] driver=${driverId} package=${p.name} amount=${p.price_cents} asaas=${pix.paymentId}`);
