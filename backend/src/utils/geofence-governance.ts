...
export function fmtLatLng(lat: number, lng: number): string {
  const f = (n: number) => (typeof n === "number" ? n.toFixed(6) : "N/A");
  return `${f(lat)}, ${f(lng)}`;
}

export function geometryQuality(geometryType: string | null | undefined): number {
  // Quanto maior, melhor
  switch (geometryType) {
    case "MultiPolygon":
      return 30;
 ...
 *   centerLat,
 *   centerLng,
 *   geofence: { geometry?: { type?: string } } | null,
 *   geofenceStatus?: number // opcional: 200/404
 * }
 */
export function canonicalScore(candidate: any): number {
...
export function pickCanonical(candidates: any[]): any | null {
